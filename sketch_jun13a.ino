...
// ======= Globals =======
float humidityThreshold = 30.0;       // Default fan-on threshold (%)
...
void setup() {
  ...
  Serial.println("WiFi AP started. Connect to: ESP_HUMIDITY");
...
void loop() {
  ...
  if (!manualMode) {
    ...
    if (humidity > humidityThreshold) {
      digitalWrite(RELAY1_PIN, HIGH); // Fan ON (if HIGH is ON)
      Serial.println("Too Humid = Fan ON");
    } else {
